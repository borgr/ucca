...
def align_nodes(nodes1, nodes2, word2word):
	...
	for node1 in gen1:
		for node2 in gen2:
			...
			if sim and (node1 not in best or sim > best[node1] or 
						(sim == best[node1] and compare(node1, node2))):
				best[node1] = sim
				mapping[node1] = node2
...
def tree_structure_aligned(n1, n2, word2word):
	...
	if len(n1.children) == longer:
		res1 = res1 + [tree_structure(n) for n in n1.children[shorter:]]
	if len(n2.children) == longer:
		res2 = res2 + [tree_structure(n) for n in n2.children[shorter:]]
...
def create_ordered_trees(p1, p2, word2word=None):
	...
	for i in range(shorter):
		...
		res2.append(t2)
	if len(top1) == longer:
		res1 = res1 + [tree_structure(n) for n in top1[shorter:]]
	if len(top2) == longer:
